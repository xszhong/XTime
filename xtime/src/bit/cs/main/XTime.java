...
import bit.cs.struct.MetaInfo;
import bit.cs.struct.TaggedToken;
import bit.cs.struct.TimeSegment;
...
public class XTime {
    ...
    private String InduceTimeMLText(String text, List<TaggedToken> taggedTokenList, List<TimeSegment> timeSegmentList, String date) throws ParseException {
        ...
        if (! timeSegmentList.isEmpty()) {
            ...
            for (int i = 1; i < timeSegmentList.size(); i++) {
                ...
                printMetaInfo(timeSegmentList.get(i-1),osw);//*******************
...
//                        refDate = dctYear+"-"+refDate.split("-")[1]+"-"+refDate.split("-")[2];
                        ...
                        properties.setMetaInfoValue(MetaInfo.MetaInfoType.MONTHMONTH, InduceTokenTriples.getTokenValue(token.getToken().split("-")[0],TokenType.MONTH)+"-"+InduceTokenTriples.getTokenValue(token.getToken().split("-")[1],TokenType.MONTH));
                    }
                    else if(token.isMonth()){
                        String value = InduceTokenTriples.getTokenValue(token.getToken().split("-")[1],TokenType.MONTH);
                        ...
                        String value1=InduceTokenTriples.getTokenValue(token1,TokenType.ORDINAL);
                        String value2=InduceTokenTriples.getTokenValue(token2,TokenType.CONTINUOUS_TIMEUNIT);
                        ...
                        String token1=token.getToken().split("-")[0];
                        String token2=token.getToken().split("-")[1];
                        String value1=InduceTokenTriples.getTokenValue(token1,TokenType.NUMBER);
                        String value2=InduceTokenTriples.getTokenValue(token2,TokenType.CONTINUOUS_TIMEUNIT);
                        properties.setMetaInfoValue(MetaInfo.MetaInfoType.CONTINUOUSTIMEUNIT,value2);
                        ...
                        String value = InduceTokenTriples.getTokenValue(cUnit,TokenType.CONTINUOUS_TIMEUNIT);
...
//                        refDate = refDate.split("-")[0] + "-" + refDate.split("-")[1] + "-" + dctDate;
            ...
            if(!modifySign.equals("")){
                ...
                if (properties.getMetaInfoValue(MetaInfo.MetaInfoType.DISCRETETIMEUNIT)!=null)
                    ...
                    switch (properties.getMetaInfoValue(MetaInfo.MetaInfoType.CONTINUOUSTIMEUNIT)){
                        case "DE":
                            modifyType= MetaInfo.MetaInfoType.DECADE;break;
                        case "Y":
                            modifyType= MetaInfo.MetaInfoType.YEAR;break;
                        case "Q":
                            modifyType= MetaInfo.MetaInfoType.QUARTER;break;
                        case "M":
                            modifyType= MetaInfo.MetaInfoType.MONTH;break;
                        case "D":
                            modifyType= MetaInfo.MetaInfoType.DAY;break;
                        case "W":
                            modifyType= MetaInfo.MetaInfoType.WEEK;break;
                        case "C":
                            modifyType= MetaInfo.MetaInfoType.CENTURY;break;
                    }
                }
    ...
    private List<Integer> identifyTimeToken(List<TaggedToken> taggedTokenList) {
        ...
        for (int i = 0; i < taggedTokenList.size(); i++) {
            ...
            Set<TokenType> tokenTypeSet = InduceTokenTriples.getTokenType(token, tag);
            taggedToken.setTokenTypeSet(tokenTypeSet);

            String value = InduceTokenTriples.getTokenValue(taggedToken.getToken(),tokenTypeSet);
            taggedToken.setValue(value);

            if (taggedToken.isHalfDay()) {
                if (token.equals("am") || token.equals("pm")) {
                    boolean isHalfDay = true;
                    if (i == 0 || (!InduceTokenTriples.isNumeral(taggedTokenList.get(i - 1)) && !InduceTokenTriples.isTime(taggedTokenList.get(i - 1))))
                        isHalfDay = false;
